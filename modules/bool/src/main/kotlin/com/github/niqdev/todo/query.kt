...
fun <T> Result<T>.toParserResult(): ParserResult<T> = this.fold({ ParserResult.Success(it) }, { ParserResult.Failure(it) })
...
inline fun <reified T> ParserResult<T>.isSuccess(): Boolean = fold({ true }, { false })
  ...
  data object Query : FilterParser {
    override fun parse(value: String): ParserResult<FilterPredicate> =
      runCatching {
        ...
        val filters =
          value.split("&").fold(emptyList<FilterPredicate.Filter>()) { xs, raw ->
            val x = raw.split("=")
...
sealed interface FilterPredicate {
  ...
  data class In(
    ...
    companion object {
      operator fun invoke(vararg filters: Filter): FilterPredicate = In(filters.asList())
    }
  }
