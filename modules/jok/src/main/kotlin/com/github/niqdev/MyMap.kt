...
class MyMapEntry<K : Comparable<K>, V> private constructor(
  ...
  override fun compareTo(other: MyMapEntry<K, V>): Int = key.compareTo(other.key)
  ...
  companion object {
    operator fun <K : Comparable<K>, V> invoke(pair: Pair<K, V>): MyMapEntry<K, V> = MyMapEntry(pair.first, MyResult(pair.second))

    operator fun <K : Comparable<K>, V> invoke(key: K): MyMapEntry<K, V> = MyMapEntry(key, MyResult.Empty)

    fun <K : Comparable<K>, V> of(
      key: K,
      value: V,
    ): MyMapEntry<K, V> = MyMapEntry(key, MyResult(value))
  }
}
...
class MyMap<K : Comparable<K>, V> private constructor(
  ...
  operator fun plus(entry: Pair<K, V>): MyMap<K, V> = MyMap(this.tree + MyMapEntry(entry))

  operator fun minus(key: K): MyMap<K, V> = MyMap(this.tree - MyMapEntry(key))

  operator fun get(key: K): MyResult<MyMapEntry<K, V>> = tree[MyMapEntry(key)]

  fun contains(): (K) -> Boolean = { key -> tree.contains(MyMapEntry(key)) }
