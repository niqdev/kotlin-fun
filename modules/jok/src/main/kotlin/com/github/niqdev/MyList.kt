...
fun <T> MyList<T>.last(): MyResult<T> = this.foldLeft<T, MyResult<T>>()(MyResult.Empty) { _, t -> MyResult(t) }
...
fun <A> MyList<A>.cons(): (A) -> MyList<A> = { head -> MyList.MyCons(head, this) }
...
fun <A, B> MyList<A>.foldRight(): (B) -> (f: (A) -> (B) -> B) -> B = { zero -> { f -> this.foldLeft<A, B>()(zero) { a, b -> f(b)(a) } } }

// ---------- 5.8 ----------

fun <A> MyList<A>.length(): Int = this.foldRight<A, Int>()(0) { { acc -> acc + 1 } }

// ---------- 5.11 ----------

fun <A> MyList<A>.reverseWithFoldLeft(): MyList<A> = this.foldLeft<A, MyList<A>>()(MyList.MyNil as MyList<A>) { acc, i -> acc.cons()(i) }
...
fun MyList<Int>.timesThree(): MyList<Int> = this.reverse().foldLeft<Int, MyList<Int>>()(MyList()) { acc, i -> acc.cons()(i * 3) }
...
fun <A, B> MyList<A>.flatMap(f: (A) -> MyList<B>): MyList<B> = this.map(f).flatten()

// ---------- 5.21 ----------

fun <A> MyList<A>.filterWithFlatMap(p: (A) -> Boolean): MyList<A> = this.flatMap { i -> if (p(i)) MyList(i) else MyList.MyNil }
...
fun <A> MyList<Option<A>>.sequence(): Option<MyList<A>> =
  when {
    this.isEmpty() -> Option.None
      ...
      this.foldLeft<Option<A>, Option<MyList<A>>>()(Option(MyList())) { result, item ->
        when (item) {
...
fun <A> MyList<Option<A>>.sequenceWithTraverse(): Option<MyList<A>> = this.traverse<Option<A>, A> { it }

// ---------- 6.12 ----------

fun <A, B> MyList<A>.traverse(): ((A) -> Option<B>) -> Option<MyList<B>> =
  { f ->
    this.map(f).foldRight<Option<B>, Option<MyList<B>>>()(Option(MyList())) { item ->
      { result ->
        result.flatMap { r -> item.map { i -> r.cons()(i) } }
      }
    }
  }
...
fun <A, B> MyList<A>.traverseWithMap2(): ((A) -> Option<B>) -> Option<MyList<B>> =
  { f ->
    this.foldRight<A, Option<MyList<B>>>()(Option(MyList())) { item ->
      { result ->
        f(item).map2<B, MyList<B>, MyList<B>>()(result) { i, r -> r.cons()(i) }
      }
    }
  }
...
fun MyList<String>.productString(): (MyList<String>) -> MyList<String> =
  { list ->
    this.foldRight<String, MyList<String>>()(MyList()) { item ->
      { result ->
        result.concat()(
          item.repeat()(list.length()).zipWith<String, String, String>()(list) { s1 ->
            { s2 ->
              s1 +
                s2
            }
...
fun <A, B> MyList<Pair<A, B>>.unzipPair(): Pair<MyList<A>, MyList<B>> = this.unzip<Pair<A, B>, A, B> { it }
...
fun <A> MyList<A>.splitAt(): (Int) -> Pair<MyList<A>, MyList<A>> =
  { index ->
    tailrec fun loop(
      i: Int,
      list: MyList<A>,
      result: Pair<MyList<A>, MyList<A>>,
    ): Pair<MyList<A>, MyList<A>> =
      when {
        i <= index ->
          when (list) {
...
fun Int.range(): (Int) -> MyList<Int> = { max -> this.unfold<Int, Int> { if (it < max) Option(it + 1 to it) else Option.None }.reverse() }
...
fun <A> MyList<A>.forAll(): ((A) -> Boolean) -> Boolean = { f -> this.foldLeft<A, Boolean>()(true) { result, item -> f(item) && result } }
  ...
  { index ->
    this.splitListAt()(index).foldLeft<MyList<A>, MyList<MyList<A>>>()(MyList()) { result, item ->
      if (item.length() > index) {
        result.concat()(item.divideByTwo())
      } else {
        result.cons()(item)
      }
    }
...
fun <A> MyList<A>.divideByTwo(): MyList<MyList<A>> = this.divide()(2)
...
fun <A> MyList<MyLazy<A>>.sequence(): MyLazy<MyList<A>> = MyLazy { this.map { it.invoke() } }

// ---------- 9.9 ----------

fun <A> MyList<MyLazy<A>>.sequenceResult(): MyLazy<MyResult<MyList<A>>> = MyLazy { this.map { MyResult.of(it) }.sequence() }
...
fun <A> MyList<A>.forEach(): ((A) -> Unit) -> Unit = { effect -> this.map(effect) }
